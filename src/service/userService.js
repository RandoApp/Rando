    ...
    forUserWithTokenWithoutSpam: function (token, ip, callback) {
        this.forUserWithToken(token, ip, function (err, user) {
                ...
                if (randos.length > config.app.limit.images) {
                    ...
                    logger.debug("[userService.forUserWithTokenWithoutSpam, ", user.email, "] Now: ", Date.now(), " last in limit image creation: ", randos[config.app.limit.images - 1].creation, " Time between Images limit: ", timeBetwenImagesLimit);
